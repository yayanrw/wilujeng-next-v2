...
- **Widget Informasi:**
  ...
  - Total transaksi hari ini (Transactions) dengan widget angka animatif (react-countup).
  - Total laba bersih hari ini (Today Net Profit) dengan widget angka animatif (react-countup).
...
- **Grafik Laba Rugi (Sales & Profit Overview):**
  - Menampilkan grafik PNL (AreaChart dengan Recharts) yang membandingkan Revenue vs Net Profit (sebelumnya Gross Profit).
...
- **List Produk:** Tabel dengan fitur search, menampilkan data produk dengan skema pagination (Load More Products). Ditambah dengan fitur Dropdown Filter berdasarkan Kategori dan Merk untuk memudahkan pencarian. Terdapat kolom "Action" dengan ikon pensil untuk mengedit atau melihat detail produk. Tombol "New" terintegrasi ke dalam panel "Edit/New Product" daripada header tabel utama. Terdapat juga tombol **"Import Products"** yang membuka modal untuk mengunggah file Excel (`.xlsx`), memungkinkan penambahan atau pembaruan produk secara massal (bulk upsert berdasarkan SKU) tanpa mengubah data harga grosir (tiering). List produk di-cache menggunakan **Upstash Redis** (`@upstash/redis`) dengan strategi Invalidation Berbasis Pola (Pattern Matching) untuk mempercepat pengambilan data katalog di menu inventaris maupun saat pencarian di menu kasir.
- **Tambah/Edit Produk:**
