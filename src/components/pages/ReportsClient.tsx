...
type ReceivableRow = { id: string; name: string; phone: string | null; totalDebt: number };

type PnlRow = { sales: number; cogs: number; profit: number };

type SupplierRow = { supplierId: string | null; supplierName: string | null; totalQty: number; purchaseValue: number };
...
export function ReportsClient() {
  ...
  async function run() {
    ...
    setError(null);
    let url = "";
    if (tab === "sales") url = `/api/reports/sales?date=${encodeURIComponent(date)}`;
    if (tab === "stock") url = `/api/reports/stock-low`;
    if (tab === "receivables") url = `/api/reports/receivables`;
    if (tab === "pnl") url = `/api/reports/pnl?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
    if (tab === "suppliers") url = `/api/reports/suppliers?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        {(["sales", "stock", "receivables", "pnl", "suppliers"] as Tab[]).map((t) => (
          <button
            key={t}
            type="button"
            className={
                ...
                <tbody>
                  {(data as SalesRow[]).map((t) => (
                    <tr key={t.id} className="border-b border-zinc-100">
                      <td className="py-2 font-mono text-xs">{String(t.id).slice(0, 8)}</td>
                ...
                <tbody>
                  {(data as StockRow[]).map((p) => (
                    <tr key={p.id} className="border-b border-zinc-100">
                      ...
                      <td className="py-2 tabular-nums">{p.minStockThreshold}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {tab === "pnl" && data ? (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-zinc-200 bg-white p-3">
                ...
                <div className="mt-1 text-base font-semibold tabular-nums">{formatIdr((data as PnlRow).sales ?? 0)}</div>
              </div>
              <div className="rounded-lg border border-zinc-200 bg-white p-3">
                <div className="text-xs text-zinc-500">COGS</div>
                <div className="mt-1 text-base font-semibold tabular-nums">{formatIdr((data as PnlRow).cogs ?? 0)}</div>
              </div>
              <div className="rounded-lg border border-zinc-200 bg-white p-3">
                <div className="text-xs text-zinc-500">Profit</div>
                <div className="mt-1 text-base font-semibold tabular-nums">{formatIdr((data as PnlRow).profit ?? 0)}</div>
              </div>
            </div>
          ) : null}
