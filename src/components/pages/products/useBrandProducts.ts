...
export function useBrandProducts() {
  ...
  const { items: brands, loading, hasMore, search, setSearch, loadMore, refresh } =
    usePaginatedList<Brand>({ fetchFn, limit: 50 });
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(body?.message || t.products.brandInUse || t.products.deleteFailed);
      } else {
        refresh();
        showToast(t.products.brandDeletedSuccess);
      }
    } catch {
