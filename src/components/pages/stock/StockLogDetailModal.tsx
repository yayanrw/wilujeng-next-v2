      ...
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <span className="block text-zinc-500">Product</span>
              <span className="font-medium">{log.productName || "-"}</span>
              <span className="block text-xs font-mono text-zinc-500">{log.productSku || "-"}</span>
            </div>
          </div>

          <div className="rounded-md border border-zinc-200 bg-zinc-50 p-4 grid grid-cols-3 gap-4 text-center">
            <div>
              <span className="block text-xs text-zinc-500 mb-1">Previous Stock</span>
              <span className="text-lg tabular-nums text-zinc-500">{log.prevStock}</span>
            </div>
            <div>
              <span className="block text-xs text-zinc-500 mb-1">Change (Qty)</span>
            ...
            <div>
              <span className="block text-xs text-zinc-500 mb-1">New Stock</span>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-zinc-100 pt-4">
            {log.unitBuyPrice !== null && (
              <div>
                <span className="block text-zinc-500">Unit Buy Price</span>
                <span className="tabular-nums font-medium">{formatIdr(log.unitBuyPrice)}</span>
              </div>
            )}
            {log.supplierId && (
              <div className="col-span-2 md:col-span-1">
                <span className="block text-zinc-500">Supplier ID</span>
                <span className="font-mono text-xs">{log.supplierId}</span>
              </div>
            )}
            <div className="col-span-2">
              <span className="block text-zinc-500">Note</span>
              <div className="mt-1 rounded-md border border-zinc-200 bg-white p-2 min-h-[60px]">
