      ...
      <div className="space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span
                className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold uppercase tracking-wide mt-1 ${
                  log.type === 'in'
              ...
              >
                {t.stock[log.type as keyof typeof t.stock]}
              </span>
            </div>
            <div>
              <span className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
          ...
          <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 p-6 flex items-center justify-between">
            <div className="text-center flex-1">
              <span className="block text-[10px] font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-1">
                {t.stock.previous}
              </span>
              <span className="text-2xl tabular-nums font-medium text-zinc-600 dark:text-zinc-300">
                {log.prevStock}
              </span>
            </div>
            <div className="flex flex-col items-center justify-center flex-1 px-4">
              <span
                className={`text-xl tabular-nums font-bold ${log.type === 'in' ? 'text-emerald-600 dark:text-emerald-400' : log.type === 'out' ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}`}
              >
            ...
            <div className="text-center flex-1">
              ...
              <span className="text-2xl tabular-nums font-bold text-zinc-900 dark:text-zinc-100">
                {log.nextStock}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-zinc-100 dark:border-zinc-800 pt-6">
            {log.unitBuyPrice !== null && (
              <div>
                ...
                <span className="tabular-nums font-medium text-sm mt-1 block">
                  {formatIdr(log.unitBuyPrice)}
                </span>
              </div>
            )}
            {log.supplierId && (
