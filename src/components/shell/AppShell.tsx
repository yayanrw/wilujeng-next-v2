...
const navItems: NavItem[] = [
  ...
  { href: "/reports", label: "Reports", icon: <BarChart3 className="h-4 w-4" />, adminOnly: true },
  { href: "/settings", label: "Settings", icon: <Settings className="h-4 w-4" />, adminOnly: true },
];
...
export function AppShell({
  ...
  const Icon =
    (Icons as unknown as Record<string, (p: { className?: string }) => ReactNode>)[
      storeIconName
    ] ?? Icons.Store;

  return (
    <div className="min-h-dvh bg-zinc-50 text-zinc-900">
      <div className={cn("grid min-h-dvh", sidebarCollapsed ? "grid-cols-[72px_1fr]" : "grid-cols-[260px_1fr]")}
      >
        <aside className="border-r border-zinc-200 bg-white">
          <div className="flex h-16 items-center gap-2 px-4">
            ...
            <button
              ...
              className={cn(
                "ml-auto rounded-md border border-zinc-200 bg-white p-2 text-zinc-700 hover:bg-zinc-50",
        ...
        <div className="flex min-w-0 flex-col">
          <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-zinc-200 bg-white px-4">
