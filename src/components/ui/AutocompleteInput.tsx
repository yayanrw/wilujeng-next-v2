...
type AutocompleteProps = {
  ...
  noMatchText?: string;
  /** Prefix text for the "will be created as new" hint (omit to hide the hint entirely) */
  createHintText?: string;
};

export const AutocompleteInput = forwardRef<HTMLInputElement, AutocompleteProps>(
  function AutocompleteInput(
    { value, onChange, options, placeholder, label, onNext, noMatchText, createHintText },
    ref,
  ) {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (filtered.length > 0) selectOption(filtered[0].name);
    ...
    const showDropdown = isOpen && (filtered.length > 0 || query.trim().length > 0);
          ...
          <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-1 text-sm shadow-md">
            {filtered.length > 0 ? (
              <>
                ...
                {createHintText && query.trim().length > 0 && !hasExactMatch && (
                  <div className="px-2 py-1.5 text-xs text-zinc-400 dark:text-zinc-500 border-t border-zinc-100 dark:border-zinc-800 mt-1 pt-1.5">
                    {createHintText}: &quot;{query}&quot;
                  </div>
                )}
              </>
            ) : (
              <div className="px-2 py-1.5 text-zinc-500 dark:text-zinc-400 flex flex-col gap-1">
                {noMatchText && <span>{noMatchText}</span>}
                {createHintText && (
                  <span className="text-xs">{createHintText}: &quot;{query}&quot;</span>
                )}
                {!noMatchText && !createHintText && <span>—</span>}
              </div>
            )}
          </div>
        )}
